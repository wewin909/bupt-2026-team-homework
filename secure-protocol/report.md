...
来到aioquic文件夹下，进入环境.\.venv\Scripts\Activate.ps1
...
4. 扩展字段
   服务器返回了 renegotiation_info、ec_point_formats 和 session_ticket 等扩展字段，表明其支持安全重协商、椭圆曲线参数协商以及后续会话票据机制。其中 `renegotiation_info` 是 RFC 5746 引入的安全重协商标志，用于把后续重协商与当前连接上下文绑定，从而防止前缀注入式重协商攻击。
...
4. session_ticket 扩展中包含具体内容
   与完整握手阶段不同，此时 `session_ticket` 扩展中已经携带票据数据，说明客户端提交了此前获得的票据，以便服务器尝试基于已有会话上下文进行恢复。
5. 其他字段
...
1. Version: TLS 1.2 (0x0303)
   说明服务器同意继续使用 TLS 1.2。
2. Session ID Length: 32
   说明服务器返回了非空会话标识，结合精简的握手流程可以判断本次连接发生了会话恢复。
3. Session ID 与客户端一致
...
这说明服务器没有重新执行完整的身份认证和密钥交换过程，而是基于先前建立的会话上下文完成了恢复连接。

因此，可以将本次连接判定为 TLS 1.2 的会话恢复。其主要依据包括：

1. Client Hello 中出现了非空的 Session ID；
2. session_ticket 扩展中携带了票据数据；
3. 服务器返回了与客户端一致的 Session ID；
4. 握手过程中未再出现完整握手所需的证书和密钥交换报文。

需要注意的是，仅从抓包中同时看到 `Session ID` 和 `session_ticket`，通常可以判断“发生了恢复连接”，但未必能够仅凭报文本身精确区分服务器内部最终是依赖 session cache 还是 session ticket 完成恢复。因此，更稳妥的结论是：本次 TLS 1.2 连接具有明显的会话恢复特征。
...
从抓包结果可以看出，TLS 1.3 完整握手在未解密情况下可直接观察到的典型流程如下：
...
2. supported_versions: TLS 1.3 (0x0304)
   该扩展明确表明客户端希望协商使用 TLS 1.3，这也是判断本次握手属于 TLS 1.3 的关键依据。
3. Session ID Length: 32
   客户端在 TLS 1.3 中仍可携带该字段，但这里的 `legacy_session_id` 主要用于兼容旧实现和中间设备，不能像 TLS 1.2 那样被直接当作恢复连接的判据。
4. Cipher Suites
...
4. Session ID
   服务器返回与客户端一致的 `legacy_session_id`，其主要意义仍是兼容性处理；TLS 1.3 的恢复机制核心依据是 `pre_shared_key`，而不是 TLS 1.2 式的 Session ID 复用。
...
4. `Server Hello` 之后的大量握手消息会被加密封装，因此抓包中可直接看到的明文握手内容明显减少。
...
从抓包结果可以看出，TLS 1.3 会话恢复在未解密情况下可直接观察到的典型流程如下：
...
TLS 1.3 会话恢复与 TLS 1.2 最大的不同在于恢复机制本身。TLS 1.2 的恢复通常表现为 `Session ID`、`session_ticket` 以及缩短的握手流程共同出现；而 TLS 1.3 则主要通过 `pre_shared_key` 实现恢复连接。
...
> Initial -> Handshake -> 1-RTT Protected Packets

其中，客户端首先发送 `QUIC Initial` 包，在其中携带 TLS 1.3 的 `Client Hello`；服务器随后返回对应握手信息，连接建立后进入 1-RTT 受保护的数据传输阶段。
...
在 QUIC 中，恢复连接机制同样建立在 TLS 1.3 的 PSK 基础之上。当客户端已经保存了之前连接建立过程中获得的票据或恢复信息后，再次连接同一服务器时，就可以在新的 `Client Hello` 中携带 `pre_shared_key`，从而发起恢复连接。若同时出现 `early_data`，则说明客户端声明了发送 0-RTT 早期数据的能力或意图，但仅凭该扩展本身不能直接断言抓包中已经实际发送了早期应用数据。

从抓包结果可以看出，QUIC 恢复连接的典型流程如下：

> Initial(pre_shared_key, early_data) -> 后续简化握手 -> 1-RTT Protected Packets
...
5. early_data 扩展存在
   该扩展表明客户端声明了发送早期数据的能力或尝试 0-RTT 的意图，这也是 QUIC 恢复连接中较有代表性的特征之一；但若要证明抓包中确实发送了 0-RTT 应用数据，还需要结合后续报文内容进一步判断。
...
因此，可以将本次连接判定为 QUIC 的恢复连接。其主要依据在于：客户端在初始握手阶段就提交了先前连接中获得的 PSK 信息，并声明了早期数据能力，这说明连接建立不再是完全从零开始，而是建立在已有安全上下文基础上的快速恢复。
...
3. 在服务器接受相关条件时，可结合 early_data 实现更早的应用数据发送；
...
QUIC 恢复连接的关键特征是 `pre_shared_key` 和 `early_data` 扩展的出现。前者说明客户端正在利用先前连接建立的安全上下文恢复连接，后者则反映出该连接具备尝试 0-RTT 的能力。与 TLS 1.2 和 TLS 1.3 相比，QUIC 在恢复连接场景下表现出更强的低时延潜力和更紧密的传输层集成能力。
...
TLS 1.2 的会话恢复通常可以从 `Session ID`、`session_ticket` 以及明显缩短的握手流程中进行判断。在恢复连接过程中，客户端通过在 `Client Hello` 中携带已有会话标识和票据，请求服务器复用先前建立的会话状态。其恢复成功的典型表现是握手流程明显缩短，且不再出现 `Certificate`、`Server Key Exchange` 和 `Server Hello Done` 等报文。
...
QUIC 的恢复连接机制本质上与 TLS 1.3 一致，同样依赖 `pre_shared_key`。但由于 QUIC 自身建立在 UDP 之上，并可结合 `early_data` 支持 0-RTT 场景，因此其恢复连接在合适条件下能够进一步减少时延，表现出更强的快速建立连接能力。

### 7.3 关键字段对比

TLS 1.2 分析时，最关键的恢复判据是非空 `Session ID`、`session_ticket` 以及握手流程缩短这几类特征的组合，而完整握手的重要标志则是 `Certificate`、`Server Key Exchange` 和 `Server Hello Done` 等报文的出现。
...
QUIC 除了具有 TLS 1.3 的 `supported_versions`、`key_share` 和 `pre_shared_key` 外，还额外包含 `quic_transport_parameters`，用于协商 QUIC 层面的连接控制参数。同时，在恢复连接中还可观察到 `early_data`，这表明连接具备尝试 0-RTT 的能力。
...
QUIC 则在 TLS 1.3 的基础上进一步优化了连接建立过程。由于其运行在 UDP 之上，避免了 TCP 三次握手带来的额外时延，并且在满足条件时可结合 `early_data` 支持 0-RTT，因此在重复连接和低时延场景下具有更明显的性能优势。
...
| 恢复连接特征 | Session ID、session_ticket、握手缩短 | pre_shared_key、key_share | pre_shared_key、quic_transport_parameters，可出现 early_data |
| 关键新增扩展 | session_ticket | supported_versions、key_share、pre_shared_key | quic_transport_parameters、early_data |
| 建连效率 | 较低 | 较高 | 最高 |

## 8. 实验结论

本实验通过对 TLS 1.2、TLS 1.3 和 QUIC 的完整握手及恢复连接过程进行抓包分析，比较了三种协议在握手流程、关键扩展字段和会话恢复机制上的差异。实验结果表明，TLS 1.2 的完整握手流程较长，能够在抓包中清晰观察到 `Certificate`、`Server Key Exchange` 和 `Server Hello Done` 等报文；其恢复连接通常可由 `Session ID`、`session_ticket` 以及明显缩短的握手流程共同判断。
...
QUIC 在密码学机制上沿用 TLS 1.3，同时将握手消息封装在 QUIC 的 `CRYPTO` 帧中，并运行于 UDP 之上。抓包结果显示，QUIC 不仅支持基于 `pre_shared_key` 的恢复连接，还可在满足条件时结合 `early_data` 支持 0-RTT，因此在低时延和高效率方面具有明显优势。
